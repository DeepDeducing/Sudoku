...
SGD to solve 6x6 Sudoku table.

This code is the same as "Deducing_6x6_pattern" and "Deducing_9x9" except their functions parameters (which we don't recommend readers to change)
...
def generate_answer_table(row, column):
    ...
    for i in range(row):
        for j in range(column):
            ...
            grid_1 = np.array([answer_table[0, 0], answer_table[0, 1], answer_table[0, 2],
                               answer_table[1, 0], answer_table[1, 1], answer_table[1, 2]])
            grid_2 = np.array([answer_table[2, 0], answer_table[2, 1], answer_table[2, 2],
                               answer_table[3, 0], answer_table[3, 1], answer_table[3, 2]])
            grid_3 = np.array([answer_table[4, 0], answer_table[4, 1], answer_table[4, 2],
                               answer_table[5, 0], answer_table[5, 1], answer_table[5, 2]])
            grid_4 = np.array([answer_table[0, 3], answer_table[0, 4], answer_table[0, 5],
                               answer_table[1, 3], answer_table[1, 4], answer_table[1, 5]])
            grid_5 = np.array([answer_table[2, 3], answer_table[2, 4], answer_table[2, 5],
                               answer_table[3, 3], answer_table[3, 4], answer_table[3, 5]])
            grid_6 = np.array([answer_table[4, 3], answer_table[4, 4], answer_table[4, 5],
                               answer_table[5, 3], answer_table[5, 4], answer_table[5, 5]])
...
def last_check(machine_table):
    ...
    examined_grid = np.array([machine_table[0,0], machine_table[0,1], machine_table[0,2],
                              machine_table[1,0], machine_table[1,1], machine_table[1,2]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[2,0], machine_table[2,1], machine_table[2,2],
                              machine_table[3,0], machine_table[3,1], machine_table[3,2]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[4,0], machine_table[4,1], machine_table[4,2],
                              machine_table[5,0], machine_table[5,1], machine_table[5,2]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[0,3], machine_table[0,4], machine_table[0,5],
                              machine_table[1,3], machine_table[1,4], machine_table[1,5]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[2,3], machine_table[2,4], machine_table[2,5],
                              machine_table[3,3], machine_table[3,4], machine_table[3,5]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[4, 3], machine_table[4, 4], machine_table[4, 5],
                              machine_table[5, 3], machine_table[5, 4], machine_table[5, 5]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0
...
for table in range(times_of_trial):
    ...
    """
      ...
      This part simply loads sets of weight matrix (as well as their corresponding sets of slope multiplie trained by deep learning in the 
      ...
      The meaning of the notations are listed below:
          ...
          --- slope_list
                  Refers to the set of slope multiplie trained by deep learning in the learning phase.
          --- weight_lists
                  Refers to the sets of weight matrix to be randomly selected/sampled in the deducing epochs to perform MWM-SGD.
          --- slope_lists
                  Refers to the sets of slope multiplie to be randomly selected/sampled in the deducing epochs to perform MWM-SGD.
    ...
    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[1]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[1]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[2]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[2]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[3]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[3]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[4]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[4]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[5]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[5]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[6]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[6]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[7]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[7]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[8]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[8]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[9]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[9]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[10]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[10]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[11]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[11]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[12]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[12]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[13]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[13]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[14]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[14]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[15]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[15]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[16]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[16]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[17]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[17]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[18]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[18]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[19]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[19]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[20]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[20]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[21]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[21]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[22]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[22]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[23]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[23]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[24]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[24]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[25]_weight_list.npy" , allow_pickle=True)
    slope_list         = np.load("self.6x6_original_1_100x100x100_30_0.000001_100m_[25]_slope_list.npy"  , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)
    ...
    from Brain_for_deducing_6x6 import *
    ...
    for times in range(numbers_missing):
        ...
        """
            ...
            In this part, under each deducing epoch, we randomly select a set of trained weight matrix and its corresponding set of slope multiplie 
