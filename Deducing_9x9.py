...
or imports listed below are different:
    ...
    --- Part B.
        --- table_size
    --- Part B-3.
        --- from Brain_for_deducing_9x9 import * 
...
This part contains the basic functions listed below:
    --- generate_asnwer_table:
            This function is to generate a (9, 9) numpy array which represents an answer table, where numbers will later 
            be dropped uniformly/randomly to generate a Sudoku table or a puzzle for machine to solve.
    --- generate_Sudoku_table:
            This function is to generate a (9, 9) numpy array which represents a Sudoku table by uniformly/randomly dropping
            visible numbers in the answer table.
            Missing numbers will be represented as 0s.
    --- return_inner:
            This function is to generate a (9, 9, 9) numpy array which represents a inner values table for the generated Sudoku table.
            ...
            it is set to be -3.5.
    --- return_resistor:
            This function is to generate a (9, 9, 9) numpy array which tells the program "not" to update inner values for visible 
    ...
    --- mandatory_pulse:
            This function is to generate two (9, 9, 9) numpy arrays which separately represent the newest inner values table
...
def generate_answer_table(row, column):
    ...
    # To generate 9x9 answer tables without exceeding maximum recursion depth in python, we instead use table generator at:
    ...
    answer_table = np.array(board, float)
...
def last_check(machine_table):
    ...
    examined_grid = np.array([machine_table[0,0], machine_table[0,1], machine_table[0,2],
                              ...
                              machine_table[2,0], machine_table[2,1], machine_table[2,2]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[3,0], machine_table[3,1], machine_table[3,2],
                              machine_table[4,0], machine_table[4,1], machine_table[4,2],
    ...
    examined_grid = np.array([machine_table[6,0], machine_table[6,1], machine_table[6,2],
                              machine_table[7,0], machine_table[7,1], machine_table[7,2],
                              machine_table[8,0], machine_table[8,1], machine_table[8,2]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0
    ...
    examined_grid = np.array([machine_table[0,3], machine_table[0,4], machine_table[0,5],
                              ...
                              machine_table[2,3], machine_table[2,4], machine_table[2,5]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[3,3], machine_table[3,4], machine_table[3,5],
                              machine_table[4,3], machine_table[4,4], machine_table[4,5],
    ...
    examined_grid = np.array([machine_table[6,3], machine_table[6,4], machine_table[6,5],
                              machine_table[7,3], machine_table[7,4], machine_table[7,5],
                              machine_table[8,3], machine_table[8,4], machine_table[8,5]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[0,6], machine_table[0,7], machine_table[0,8],
                              machine_table[1,6], machine_table[1,7], machine_table[1,8],
                              machine_table[2,6], machine_table[2,7], machine_table[2,8]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[3,6], machine_table[3,7], machine_table[3,8],
                              machine_table[4,6], machine_table[4,7], machine_table[4,8],
                              machine_table[5,6], machine_table[5,7], machine_table[5,8]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0

    examined_grid = np.array([machine_table[6,6], machine_table[6,7], machine_table[6,8],
                              machine_table[7,6], machine_table[7,7], machine_table[7,8],
                              machine_table[8,6], machine_table[8,7], machine_table[8,8]])
    for i in range(examined_grid.shape[0]):
        target_number = examined_grid[i]
        if (np.count_nonzero(examined_grid[:] == target_number) != 1):
            return 0
...
This part is the start of test. Here we define:
    ...
    --- numbers_missing:
            ...
            For example, for 9x9 Sudoku table, if it is 64, it means there will be 64 numbers missing, lefting only 17 numbers (17 given).
            If it is 81, then it is a blank Sudoku table (0 given).
    --- answer_table:
            A 2-D numpy array.
...
for table in range(times_of_trial):
    ...
    """
      ...
      The meaning of the notations are listed below:
          ...
          --- self.{9x9}_{original}_{200}_{100x100x100}_{30}_{0.00001}_{100m}_{[1]}_{weight_list}
                  The meaning of each bracket is listed below:
                      --- {9x9}
                              Meaning this neural network is trained to solve each row, column and grid in 9x9 Sudoku.
                      --- {original}
                      ...
                      --- {200}
                              Meaning the sample batch in the learning phase for this trained neural network is 200 per each learning epoch.
                      --- {100x100x100}
                      ...
                      --- {0.00001}
                              Meaning the learning rate in the learning phase for this trained neural network is 0.00001.
                      --- {100m}
                      ...
                      --- {[1]}
                              Meaning the label of this trained neural network under the training condition {original}_{200}_{100x100x100}_{30}_{0.00001}_{100m}.
    ...
    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[1]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[1]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[2]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[2]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[3]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[3]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[4]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[4]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[5]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[5]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[6]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[6]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[7]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[7]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[8]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[8]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[9]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[9]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[10]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[10]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[11]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[11]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[12]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[12]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[13]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[13]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[14]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[14]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[15]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[15]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[16]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[16]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[17]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[17]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[18]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[18]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[19]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[19]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)

    weight_list        = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[20]_weight_list.npy"  , allow_pickle=True)
    slope_list         = np.load("self.9x9_original_200_100x100x100_30_0.00001_100m_[20]_slope_list.npy"   , allow_pickle=True)
    weight_lists.append(weight_list)
    slope_lists.append(slope_list)
